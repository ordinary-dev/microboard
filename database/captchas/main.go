...
func CreateCaptcha(ctx context.Context, db *pgxpool.Pool) (*Captcha, error) {
	...
	err := db.QueryRow(ctx, query, answer).Scan(&captcha.ID)

	return &captcha, err
}

func GetAnswer(ctx context.Context, db *pgxpool.Pool, captchaID uuid.UUID) (answer string, err error) {
	query := `SELECT answer FROM captchas WHERE id = $1`
	err = db.QueryRow(ctx, query, captchaID).Scan(&answer)
	return
}

// Returns `true` if the answer is valid.
// Captcha will be deleted and cannot be reused.
func ValidateCaptcha(ctx context.Context, db *pgxpool.Pool, captchaID uuid.UUID, answer string) (bool, error) {
    ...
    `
	cmdTag, err := db.Exec(ctx, query, captchaID, answer)
